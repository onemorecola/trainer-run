...
> **Автоопределение**: при обычном запуске trainer-run сам замечает свежие
> FLiNG-трейнеры (в имени есть дата сборки вида `v20260619`) и, если
> в префиксе игры нет настоящего .NET (только Wine Mono), предлагает
> поставить `dotnet40` — один раз, дальше запускает трейнер как обычно.
> Старые трейнеры (без даты в имени) проверку не проходят и вопросов не задают.
